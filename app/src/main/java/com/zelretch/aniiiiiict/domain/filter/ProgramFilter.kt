...
class ProgramFilter {
    ...
    private fun applyMediaFilter(
        program: ProgramWithWork,
        filterState: FilterState,
    ): Boolean = filterState.selectedMedia.isEmpty() || program.work.media in filterState.selectedMedia

    private fun applySeasonFilter(
        program: ProgramWithWork,
        filterState: FilterState,
    ): Boolean = filterState.selectedSeason.isEmpty() || program.work.seasonName in filterState.selectedSeason

    private fun applyYearFilter(
        program: ProgramWithWork,
        filterState: FilterState,
    ): Boolean = filterState.selectedYear.isEmpty() || program.work.seasonYear in filterState.selectedYear
    ...
    private fun applyStatusFilter(
        program: ProgramWithWork,
        filterState: FilterState,
    ): Boolean = filterState.selectedStatus.isEmpty() || program.work.viewerStatusState in filterState.selectedStatus
    ...
    private fun applyAiredFilter(
        program: ProgramWithWork,
        filterState: FilterState,
    ): Boolean = !filterState.showOnlyAired || !program.firstProgram.startedAt.isAfter(LocalDateTime.now())
