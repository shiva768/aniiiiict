...
open class ViewModelTestWrapper<T : BaseUiState>(
    ...
    private fun getUiState(): MutableStateFlow<T> = uiStateField.get(viewModel) as MutableStateFlow<T>

    override fun setUiStateForTest(state: T) {
        getUiState().value = state
    }

    override fun setErrorForTest(error: String?) {
        val currentState = getUiState().value
        // BaseUiStateのコピーメソッドを使用（リフレクションで型安全に）
        try {
            val copyMethod = currentState::class.java.getMethod("copy", String::class.java)
            @Suppress("UNCHECKED_CAST")
            getUiState().value = copyMethod.invoke(currentState, error) as T
        } catch (e: Exception) {
            // Fallback: 直接フィールドを更新
    ...
    override fun setLoadingForTest(isLoading: Boolean) {
        ...
        try {
            ...
            @Suppress("UNCHECKED_CAST")
            getUiState().value = copyMethod.invoke(currentState, isLoading) as T
        } catch (e: Exception) {
            // Fallback: 直接フィールドを更新
    ...
    override fun resetToInitialState() {
        getUiState().value = initialStateFactory()
    }
